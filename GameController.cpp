...
{
	SoundMapType::const_itator p = m_soundMap.find(soundID);
	if (p != m_soundMap.end())
...
void GameController::displayGamePlay()
{
	...
	for (std::set<GraphObject*>::itator it = graphObjects.begin(); it != graphObjects.end(); it++)
	{
		GraphObject* cur = *it;
		if (cur->isVisible())
		{
			cur->animate();
			DrawMapType::const_itator p = m_drawMap.find(cur->getID());
			if (p != m_drawMap.end())
